...
sub driver_post ($$) {
    ...
    if ($userid) {
       # my $status =       masked by guoyan 2019.12.11
...
sub submitr ($$$$$$) {
    ...
    my $file = "bits.c";   #added by guoyan 2019.12.11
    my $buffer;
    ...
    print SERVER  "GET /$course/submitr.pl/?userid=$userid&lab=$lab&result=$enc_result&submit=submit HTTP/1.0\r\n\r\n";
     ...
     #####################added by guoyan 2019.12.11
    open(FILE,"$file") or die("open failed for $!");
   # binmode(FILE);
    seek(FILE,0,0);
    while(!eof(FILE)){
            read(FILE,$buffer,100);
            #send(SERVER,$buffer,0);
            print SERVER $buffer;
            }#while
    close(FILE);
    ##############################################masked by guoyan 2019.12.11
